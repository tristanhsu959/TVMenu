...
class BuygoodPosSale00 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'buygood:pos-sale00 {argStDate?} {argEndDate?}';
     ...
     * @var string
     */
    protected $description = 'Buygood Pos Sale00';
    ...
    public function handle()
    {
		...
		{
			Log::channel('commandLog')->error('Fetch buygood sale00 : ' . $e->getMessage(), [ __class__, __function__, __line__]);
	...
	{
		$this->info("Fetch buygood sale00 start -----" . now());
		$this->info("Fetch {$stDate} to {$endDate}");
		Log::channel('commandLog')->info("Fetch buygood sale00 start" . now(), [ __class__, __function__, __line__]);
		...
		Log::channel('commandLog')->info("Fetch buygood sale00 completed" . now(), [ __class__, __function__, __line__]);
	}
