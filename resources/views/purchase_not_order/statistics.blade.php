...
@push('styles')
    <link href="{{ HelperLib::versionAsset('styles/purchase_not_order/list.css') }}" rel="stylesheet">
@endpush

@push('scripts')
    <script src="{{ HelperLib::versionAsset('scripts/purchase_not_order/list.js') }}" defer></script>
@endpush

@section('content')
<!-- Search panel -->
<dialog x-data="searchNotOrder(@js($viewModel->searchFormData()))" id="searchPanel" class="right">
	...
	@csrf
		...
		<div x-show="searchData.type == 'product'">
			<div class="field middle-align">
				<nav class="wrap">
...
<div x-data="{response:@js($viewModel->responseData())}" class="content-wrapper">
	...
	<template x-if="response.status && response.isInit">
		...
		<section class="container">
			...
			<pre class="red-border">未訂查詢判別條件：<br/>全部符合：所有產品皆無訂購量<br/>任一符合：至少一項產品無訂購量</pre>
		</section>
	</template>
